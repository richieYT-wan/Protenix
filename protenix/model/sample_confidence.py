...
def _compute_full_data_and_summary(
    ...
    # >>> ADDED: ipSAE / min_ipSAE
    # TODO: THIS WORKS (a K:V pair for minIPSAE gets written to JSON) ; need to check value / correctness
    summary_confidence.update(
        calculate_chain_based_ipsae(
...
def calculate_chain_based_ipsae(
    pae: torch.Tensor,
    pae_prob: torch.Tensor,
    has_frame: torch.BoolTensor,
    asym_id: torch.LongTensor,
    ...
    """
    Compute ipSAE (interface predicted Structure-Aligned Error) scores per chain pair,
    following Dunbrack lab (https://github.com/DunbrackLab/IPSAE).

    For each ordered chain pair (a, b):
      - Define a residue mask on chain a: residues i in a such that the
        number of residues j in chain b with PAE[i, j] < pae_cutoff is > 0.
      - The TM normalization d0 is computed using n_aligned = #{ valid (i,j) pairs }
        per row i; here we use the count of residues in b passing the cutoff for each i.
      - For each i in a, compute mean over j in b (with PAE[i,j] < cutoff) of the
        per-bin TM weight: sum_bin prob[i,j,bin] * 1/(1 + (bin_center/d0_i)^2)
      - ipSAE[a,b] = max over valid i in a of that mean.

    Args:
        pae: [..., N_token, N_token] expected PAE (Å)
        pae_prob: [..., N_token, N_token, N_bins] PAE bin probabilities
        has_frame: [N_token]
        asym_id: [N_token]
        min_bin, max_bin, no_bins: PAE bin params
        pae_cutoff: cutoff in Å (default 10.0)

    Returns:
        dict with:
          - chain_pair_ipsae: [..., N_chain, N_chain] ipSAE per ordered chain pair
          - min_ipsae: [...] min over all distinct ordered chain pairs
    """
    ...
    N_chain = int(asym_id.max().item()) + 1
    batch_shape = pae_prob.shape[:-3]
    device = pae_prob.device

    bin_center = get_bin_centers(min_bin, max_bin, no_bins).to(device)  # [N_bins]

    chain_pair_ipsae = torch.zeros(batch_shape + (N_chain, N_chain), device=device)

    # pae shape: [..., N_tok, N_tok]; broadcast batch over all batch dims
    for a in range(N_chain):
        mask_a = (asym_id == a) & has_frame  # only rows with frame contribute to TM
        if mask_a.sum() == 0:
            continue
        for b in range(N_chain):
            ...
            # Sub-tensors
            # pae_sub: [..., n_a, n_b]
            pae_sub = pae[..., mask_a, :][..., :, mask_b]
            # prob_sub: [..., n_a, n_b, N_bins]
            prob_sub = pae_prob[..., mask_a, :, :][..., :, mask_b, :]

            # Mask of pairs with PAE < cutoff
            valid_pair = pae_sub < pae_cutoff  # [..., n_a, n_b]

            # n_aligned per row i (chain a): how many j in b have PAE<cutoff
            n_per_row = valid_pair.sum(dim=-1).float()  # [..., n_a]

            # For ipSAE, d0 should be computed per i using n_per_row[i].
            # Use clamp(min=1) so d0 is defined; rows with 0 valid pairs will
            # be excluded by masking before the max.
            n_clamped = n_per_row.clamp(min=1.0)
            # d0 = 1.24 * (max(n,19) - 15)^(1/3) - 1.8
            n_eff = torch.clamp(n_clamped, min=19.0)
            d0 = 1.24 * (n_eff - 15.0) ** (1.0 / 3.0) - 1.8  # [..., n_a]

            # Per-row, per-bin weight: 1 / (1 + (bin_center / d0_i)^2)
            # bin_center: [N_bins], d0: [..., n_a] -> [..., n_a, 1] then broadcast
            d0_exp = d0.unsqueeze(-1)  # [..., n_a, 1]
            per_bin_w = 1.0 / (1.0 + (bin_center / d0_exp) ** 2)  # [..., n_a, N_bins]
            # Expand to pair: [..., n_a, 1, N_bins]
            per_bin_w = per_bin_w.unsqueeze(-2)

            # Pairwise TM contribution = sum_bin prob[i,j,bin] * w[i,bin]
            pair_tm = (prob_sub * per_bin_w).sum(dim=-1)  # [..., n_a, n_b]

            # Zero out invalid pairs, then mean over valid j per row i
            pair_tm = pair_tm * valid_pair.float()
            row_score = pair_tm.sum(dim=-1) / (n_per_row + eps)  # [..., n_a]

            # Exclude rows with no valid pairs from the max
            row_valid = n_per_row > 0  # [..., n_a]
            # Replace invalid rows with -inf so they are not picked by max
            row_score = torch.where(
                row_valid,
                row_score,
                torch.full_like(row_score, float("-inf")),
            )

            # If no row valid at all, set ipsae to 0
            any_valid = row_valid.any(dim=-1)  # [...]
            max_score = row_score.max(dim=-1).values  # [...]
            max_score = torch.where(
                any_valid, max_score, torch.zeros_like(max_score)
            )

            chain_pair_ipsae[..., a, b] = max_score

    # min over distinct ordered pairs (off-diagonal)
    off_diag_mask = ~torch.eye(N_chain, dtype=torch.bool, device=device)
    # Apply mask along last two dims
    if N_chain >= 2:
        flat = chain_pair_ipsae[..., off_diag_mask]  # [..., N_chain*(N_chain-1)]
        min_ipsae = flat.min(dim=-1).values
    else:
        min_ipsae = torch.zeros(batch_shape, device=device)

    return {
        "chain_pair_ipsae": chain_pair_ipsae,
        "min_ipsae": min_ipsae,
    }
